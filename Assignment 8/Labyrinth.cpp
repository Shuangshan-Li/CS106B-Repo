/*  Description: Return whether a series of moves can lead to an escape from the Labyrinth
 *
**/
#include "Labyrinth.h"
...
bool isPathToFreedom(MazeCell* start, const string& moves) {
    MazeCell* cur = start;
    //Use a set to store each item that is found along the moves
    Set<Item> bag;
    //First check whether the starting point offers any item
    if (cur->whatsHere != Item::NOTHING) {
        bag.add(cur->whatsHere);
    }
    //Make moves and collect items
    for (char move:moves){
        if (move == 'N') {
            //If a move leads us to an invalid position, just return false
    ...
    //If 3 items are collected, we can excape!
    return bag.size() == 3;
}
