/*  Description: This file explores how to delete, print, form, and slice linked lists
 *
**/
...
void deleteNucleotides(Nucleotide* dna) {
    ...
    while (dna != nullptr) {
        //Hold reference to the node next to the current one
...
Nucleotide* toStrand(const string& str) {
    //use head ptr to hold reference to the first node
    //use tail to grow the linked list
    Nucleotide* head = nullptr;
    Nucleotide* tail = nullptr;
    for (char c: str) {
        Nucleotide* node = new Nucleotide{c, nullptr, nullptr};
        if (head == nullptr) {
            //change where the actual head ptr is pointing at
            head = node;
        } else {
            tail->next = node;
            node->prev = tail;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    int lengthOfDna = lengthOf(dna);
    int lengthOfTarget = lengthOf(target);

    //If target is empty, we can always find it at the begining of dna
    if (lengthOfTarget == 0) return dna;
    //If target is longer that dna, it is impossible to find.
    if (lengthOfDna < lengthOfTarget) return nullptr;

    Nucleotide* cur = dna;
    for (int i = 0; i <= lengthOfDna - lengthOfTarget ; i++) {
        //If two node matches in value, check whether a target can be found in dna, starting at this position
        if (cur->value == target->value) {
            Nucleotide* ptr1 = cur;
            Nucleotide* ptr2 = target;
            while (ptr1 != nullptr && ptr2 != nullptr) {
                if (ptr1->value != ptr2->value) break;
                ptr1 = ptr1->next;
                ptr2 = ptr2->next;
            }
            if (ptr2 == nullptr) return cur;
        }
        cur = cur->next;
    }
    return nullptr;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    int lengthOfTarget = lengthOf(target);
    if (lengthOfTarget == 0) return true;
    
    //Get the head and tail of the segment that is to be removed from dna
    Nucleotide* head = findFirst(dna, target);
    if (head == nullptr) return false;

    Nucleotide* tail = head;
    for (int i = 0; i < lengthOfTarget - 1; i++) {
        tail = tail->next;
    }
    
    //If the slice starts at the begening of dna, we need to change where dna is pointing at
    if (head == dna) {
        Nucleotide* newHead = tail->next;
        tail->next = nullptr;
        if (newHead != nullptr) {
        ...
        Nucleotide* beforeHead = head->prev;
        Nucleotide* afterTail = tail->next;

        head->prev = nullptr;
        tail->next = nullptr;

        beforeHead->next = afterTail;
        //afterTail might be a nullptr. then nullptr->prev causes error!
        if (afterTail != nullptr) {
            afterTail->prev = beforeHead;
        }
    }
...
STUDENT_TEST("deleteNucleotides cleans up a sequence of indential values") {
...
STUDENT_TEST("fromDNA handles a sequence of identical values") {
    ...
    EXPECT_EQUAL(fromDNA(one), "AAAA");
    deleteNucleotides(one);
}

STUDENT_TEST("toStrand handles a string a multiple identical characters") {
...
STUDENT_TEST("findFirst return the first appearance inseatd of the second") {
    Nucleotide* dna    = toStrand("ABCTABCTA");
    Nucleotide* target = toStrand("BCT");

    Nucleotide* cur = dna;
    cur = cur->next;
    EXPECT_EQUAL(findFirst(dna, target), cur);

    deleteNucleotides(dna);
    deleteNucleotides(target);
}

STUDENT_TEST("spliceFirst works with characters that are not in the examples") {
    Nucleotide* dna    = toStrand("1234567");
    Nucleotide* target = toStrand("4567");

    /* In this case, we should change where DNA is pointing. */
    Nucleotide* originalDNA = dna;
    EXPECT(spliceFirst(dna, target));
    EXPECT_EQUAL(fromDNA(dna), "123");
