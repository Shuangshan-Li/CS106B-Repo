/*  Description: This function takes as inputs a road network(a bunch of connected cities), number of cities
 *               we can stockpile, and return whether we can cover all the cities in crisis, and if so, tell
 *               us what that arrangement looks like
*/
#include "DisasterPlanning.h"
#include "GUI/SimpleTest.h"
using namespace std;

//We need more information in each call stack, so we use a wrapper function, which
//allows us to track cities that are still uncovered in each call stack.
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                               int numCities,
                               Set<string>& supplyLocations,
                               const Set<string>& uncoverd
                               ) {
    if (numCities == 0 && !uncoverd.isEmpty()) {
        return false;
    }
    if (uncoverd.isEmpty()) {
        return true;
    }
    string currentCity = uncoverd.first();
    Set<string> currentOptions = roadNetwork[currentCity] + currentCity;
    //Explore every option that can cover the city we care about in this call stack
    for (string city: currentOptions) {
        Set<string> coveredBy = roadNetwork[city] + city;
        Set<string> newUncoverd = uncoverd - coveredBy;
        //If a certain option can lead us to a evetually successful arrangement
        //We add this choice to the final result
        if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations, newUncoverd)) {
            supplyLocations += city;
            return true;
        }
    }
    //For the city we care about in this call stack, if no choices can evetually lead us to success, we just return false :(
    return false;
}
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            ...
                            Set<string>& supplyLocations) {
    if (numCities < 0) {
        error("numCities cannot be negative bro!");
    }
    ...
    return canBeMadeDisasterReadyRec(roadNetwork, numCities, supplyLocations, uncovered);
}
...
STUDENT_TEST("Works for three interconnected cities") {
    ...
    EXPECT(canBeMadeDisasterReady(map, 1, locations));
    EXPECT_EQUAL(locations.size(), 1);
}
