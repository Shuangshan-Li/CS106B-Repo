/*  Description: This function takes in as input a group of doctors, each with
 *               their available working hours and a group of patients, each with
 *               how much time they need from a doctor (Both represeted as map). With
 *               these inputs, this function return whether it is possible to macth
 *               doctors and patient successfully, and if so, tell what that arrangement
 *               looks like.
*/
...
bool canAllPatientsBeSeen(const Map<string, int>& doctors,
    ...
    // There are still patients who have no doctors to go to
    if (doctors.isEmpty() && !patients.isEmpty()) {
        return false;
    }
    // All patients are seen, notice that we don't care if all doctors have used up their working hours
    if (patients.isEmpty()) {
        return true;
    }

    // In each recrusive step, try to arrange for one patient
    string patient = patients.firstKey();
    int patientHours = patients[patient];
    // Update patients who have not been seen
    Map<string, int> remainingPatients;
    for (string p: patients) {
        if (p != patient) {
            remainingPatients[p] = patients[p];
        }
    }
    // Explore each choice, namely, send the patient to see each doctor
    for (string d: doctors) {
        int doctorHours = doctors[d];
        // Constraints: Only doctors who have enough time can see this patient
        if (doctorHours >= patientHours) {
            Map<string, int> remainingDoctors;
            if (doctorHours - patientHours == 0) {
                for (string d1: doctors){
                    if (d1 != d) {
                        remainingDoctors[d1] = doctors[d1];
                    }
                }
                ...
                for (string d1: doctors) {
                    if (d1 != d){
                        ...
                        remainingDoctors[d1] = doctorHours - patientHours;
                    }
                }
            }
            //If this choie can eventually lead us to a successful arrangement, we add
            // it to the final result.
            if (canAllPatientsBeSeen(remainingDoctors, remainingPatients,schedule)) {
                schedule[d].add(patient);
                return true;
            }
        }

    }
    //For this patient, if no doctor can see him/her, that means we cannot have a successful arrangement
    return false;
}
...
STUDENT_TEST("Can schedule three doctors and four patients") {
    Map<string, Set<string>> schedule;
