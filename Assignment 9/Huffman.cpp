/*  Description: I don't know what to say. This really is a challenging but rewarding journey!
 *               Thank you Keith! Thank you Chase! Thank you Sophie! Best class I have ever taken!
**/
#include "Huffman.h"
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    //First create a map about each character's frequency
    HashMap<char, int> freq;
    for (char c: str) {
        freq[c]++;
    }
    //If there is 1 or 0 key, we cannot make a tree!
    if (freq.size() <= 1) error("At least two different characters are needed!");

    PriorityQueue<EncodingTreeNode*> pq;

    for (char c: freq.keys()) {;
        pq.enqueue(new EncodingTreeNode{c, nullptr, nullptr}, freq[c]);
    }
    //Repeatedly take out two least freqent char and form a subtree untile we have only one tree.
    while (pq.size() != 1) {
        EncodingTreeNode* root = new EncodingTreeNode;
        EncodingTreeNode* zeroChar = pq.peek();
        double zeroFreq = pq.peekPriority();
        pq.dequeue();
        root->zero = zeroChar;
        EncodingTreeNode* oneChar = pq.peek();
        double onefreq = pq.peekPriority();
        pq.dequeue();
        root->one = oneChar;
        pq.enqueue(root, zeroFreq + onefreq);
    }
    return pq.dequeue();
}
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while (!bits.isEmpty()) {
        //Each time start from root node, go deep until we are at a leave node!
        EncodingTreeNode* cur = tree;
        while (cur->zero != nullptr) {
            Bit a = bits.dequeue();
            if (a == 0) {cur = cur->zero;}
            else {cur = cur->one;}
        }
        //append the character at this leave node.
        ans += cur->ch;
    }
    return ans;
}

//a healper function that based on a given huffmanTree, build a hashmap about each char's bit representation
void buildTable(EncodingTreeNode* tree, HashMap<char, Queue<Bit>>& table, const Queue<Bit>& soFar) {
    if (tree->zero == nullptr) {
        table[tree->ch] = soFar;
        return;
    }
    Queue<Bit> newsoFar = soFar;
    newsoFar.enqueue(Bit(0));
    buildTable(tree->zero, table, newsoFar);
    newsoFar = soFar;
    newsoFar.enqueue(Bit(1));
    buildTable(tree->one, table, newsoFar);
}
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    Queue<Bit> ans;
    //using the table, put each char's bit representation into ans
    for (char c: str) {
        //we don't want to change the map -> table[c].dequeue() is wrong!!!!!
        Queue<Bit> temp = table[c];
        while (!temp.isEmpty()) {
            ans.enqueue((temp.dequeue()));
        }
    }
    return ans;
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    //if this bit is 0, we are at a leave
    if (bit == Bit(0)) {
        return new EncodingTreeNode{leaves.dequeue(), nullptr, nullptr};
    }
    //else we build a root and go deep to build its zero subtree and then one subtree
    //this is dfs
    EncodingTreeNode* root = new EncodingTreeNode;
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    //else we find its zero subtree's bit representation and then its one subtree's.
    bits.enqueue(1);
...
HuffmanResult compress(const string& text) {
    ...
    //get the tree's representation
    encodeTree(tree, bits, leaves);

    HuffmanResult result{bits, leaves, encodeText(text, tree)};
    //no memory leak!
    deleteTree(tree);
...
STUDENT_TEST("huffmanTreeFor builds tree for two non-alphabet characters.") {
