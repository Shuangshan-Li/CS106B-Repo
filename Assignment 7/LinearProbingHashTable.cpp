/*  Description: This is an implementation of hashtable using linear probing
 *
**/
...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<std::string> hashFn) {
    ...
    curSize = 0;
    elems = new Slot[capacity]{{"", SlotType::EMPTY}};
}
...
//Insert a string into a certain position
bool LinearProbingHashTable::insert(const string& elem) {
    //check if it is possible to insert
    if (curSize == capacity || contains((elem))) return false;
    int pos = hash(elem);
    //keep walking until we find a cell that is not filled
    while (elems[pos].type == SlotType::FILLED) {
        pos = (pos + 1) % capacity;
    }
    //insert at this cell
    insertAt(pos, elem);
...
//find whether a string is in this hashtable
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    if (pos == -1) return false;
    //else make the cell a Tombstone!
...
void LinearProbingHashTable::printDebugInfo() const {
    for (int i = 0; i < capacity; i++) {
        cout << "value: " << elems[i].value << "type: " << elems[i].type << endl;
    }
}

void LinearProbingHashTable::insertAt(int pos, const string& elem) {
    elems[pos].type = SlotType::FILLED;
    elems[pos].value = elem;
}

//Find the position of a string, else return -1
int LinearProbingHashTable::find(const string& elem) const{
    int pos = hash(elem);
    if (elems[pos].type == SlotType::FILLED and elems[pos].value == elem) return pos;
    if (elems[pos].type == SlotType::EMPTY) return -1;
    int originalPos = pos;
    pos = (pos + 1) % capacity;
