/*  Description: This is an implementation of hashtable using robinhood hashing
**/
...
//Insert a string into this hashtable and return whether the insertion is successful
bool RobinHoodHashTable::insert(const string& elem) {
    if (curSize == capacity || contains(elem)) return false;
    int pos = hash(elem);
    Slot node{elem, 0};
    //If this cell is empty, we just insert
    if (elems[pos].distance == EMPTY_SLOT) {
        elems[pos] = node;
    } else {
        //else keeo walking to find a cell that we can insert
        while (elems[pos].distance != EMPTY_SLOT) {
            //If the element at a cell is closer to 'home' than we are, we rest at this position
            //and insert that element
            if (elems[pos].distance < node.distance) {
                swap(elems[pos], node);
            }
            pos = (pos + 1) % capacity;
            node.distance++;
...
//remove an elemenet from the hashtable and return whether the operation is successful
bool RobinHoodHashTable::remove(const string& elem) {
    int pos = find(elem);
    //If ths element is not in this hastable, just return false
    if (pos == -1) return false;
    //Make this cell empty and do backward shifts.
    elems[pos].distance = EMPTY_SLOT;
    int nextPos = (pos + 1) % capacity;
    //if this cell has an element away from 'home' or is empty, we move it backward
    while (elems[nextPos].distance != EMPTY_SLOT && elems[nextPos].distance != 0) {
        swap(elems[pos], elems[nextPos]);
        elems[pos].distance--;
        pos = nextPos;
        nextPos = (pos + 1) % capacity;
    }
    curSize--;
    return true;

}
//Find the index of a string, otherwise return -1
int RobinHoodHashTable::find(const string& elem) const{
    int pos = hash(elem);
    if (elems[pos].distance == EMPTY_SLOT) return -1;
    if(elems[pos].element == elem) return pos;
    int distance = 1;
    int originalPos = pos;
    pos = (pos + 1) % capacity;
    while (pos != originalPos) {
        if (elems[pos].distance == EMPTY_SLOT || elems[pos].distance < distance) return -1;
        if (elems[pos].element == elem) return pos;
        distance++;
        pos = (pos + 1) % capacity;
    }
...
void RobinHoodHashTable::insertAt(int pos, int distance, const string& elem) {
    elems[pos] = {elem, distance};
}

void RobinHoodHashTable::printDebugInfo() const {
    for (int i = 0; i < capacity; i++) {
        cout << "Value: " << elems[i].element << " Distance: " << elems[i].distance << endl;
    }
}
