/*  Description: This function takes as inputs a Vector of Vectors of sorted DataPoint and combine them
 *               into one sort sequence.
 *
**/
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    //If there is only one DataPoint, it is already combined
    if (sequences.size() == 1) {
        return sequences[0];
    }
    //If there is no element, it is already combined as well
    if (sequences.size() == 0) {
        return {};
    }
    //Chop the vector into roughly two halves and combine them respectively
    int half = sequences.size() / 2;
    Vector<DataPoint> firstHalf = combine(sequences.subList(0, half));
    Vector<DataPoint> secondHalf = combine(sequences.subList(half));

    Vector<DataPoint> result;
    int i = 0;
    int j = 0;
    //While neither half's DataPoints are all used to form the new sequence,
    //compare and combine one by one
    while (i < firstHalf.size() && j < secondHalf.size()) {
        if (firstHalf[i].weight < secondHalf[j].weight) {
            result += firstHalf[i];
            ++i;
        } else {
            result += secondHalf[j];
    ...
    //If one half is used up, concatenate the other half to result
    if (i < firstHalf.size()) {
        result += firstHalf.subList(i);
    }
    if (j < secondHalf.size()) {
        result += secondHalf.subList(j);
    }
    return result;
...
STUDENT_TEST("Merge one 4-item sequence and one empty sequence") {
    ...
    EXPECT_EQUAL(merged[1].weight, two[1].weight);
    EXPECT_EQUAL(merged[2].weight, two[2].weight);
    EXPECT_EQUAL(merged[3].weight, two[3].weight);
}
