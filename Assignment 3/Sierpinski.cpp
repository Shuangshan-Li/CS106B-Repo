/**
 * Given coordinates of a sierpinski triangle and its order
 * this function draw it on a window. Figuring out the math behind
 * sierpinski triangles' corners is the key.
 */
...
void drawSierpinskiTriangle(GWindow& window,
                            ...
                            int order) {
    if (order < 0) {
        error("Order must be equal to or larger than 0!");
    }
    if (order == 0) {
        drawTriangle(window, x0, y0, x1, y1, x2, y2);
    } else {
        //The mid point of (x0, y0) (x1, y1) is ((x0 + x1)/2, (y0 + y1)/2)
