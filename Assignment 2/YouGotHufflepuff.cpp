/* File: YouGotHufflepuff.cpp
 * Description: Takes scores of answering questions and return based on the
 *              scores which person does the subject most resembles.
 */
...
Question randomQuestionFrom(Set<Question>& questions) {
    if (questions.isEmpty()) {
        error("Provided question set is empty!");
    }
...
Map<char, int> scoresFrom(const Map<Question, int>& answers) {
    ...
    evaluation[1] = -2;
    Map<char, int> result;
    for (const Question& question: answers) {
        int multiplier = evaluation[answers[question]];
        for (char factor: question.factors) {
            result[factor] += question.factors[factor] * multiplier;
        }
    }
    return result;
}
...
Map<char, double> normalize(const Map<char, int>& scores) {
    double squareSum = 0;
    for (char key: scores) {
        squareSum += scores[key] * scores[key];
    }
    double divisor = sqrt(squareSum);
    if (divisor == 0) {
        error("Cannot use zero as a divisor!");
    }

    Map<char, double> normalizedScores;
    for (char key: scores) {
        normalizedScores[key] = scores[key] / divisor;
    }
...
Person mostSimilarTo(const Map<char, int>& scores, const Set<Person>& people) {
    if (people.isEmpty()) {
        error("There are no person to compare to!");
    }
    Map<char, double> normalizedInput = normalize(scores);
    Map<double, Person> peopleMap;
    for (const Person& person: people) {
        Map<char, double> normalizedPerson = normalize(person.scores);
        peopleMap[cosineSimilarityOf(normalizedInput, normalizedPerson)] = person;
    }
    return peopleMap[peopleMap.keys().last()];
